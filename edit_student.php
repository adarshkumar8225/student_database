...
if(isset($_POST['clgid'])){
	$studentid = $_POST['clgid'];
	$q = "UPDATE `studentdata1` SET `mname` = '".$_POST['mname']."', `cpi` = '".$_POST['cpi']."', `sem` = '".$_POST['sem']."', `prg_code` = '".$_POST['prg_code']."', `email` = '".$_POST['email']."', `password` = '".$_POST['password']."' WHERE `clgid` = '".$studentid."'";
	...
	$q = "SELECT * FROM `studentdata1` WHERE `clgid`='".$_GET['clgid']."'";
...
echo('<input type="hidden" name="clgid" value="'.$_GET['clgid'].'">');
echo('<tr><td>Collage id :</td><td><input type="text" name="clgid" value="'.$a['clgid'].'"></td></tr>');
echo('<tr><td>Name :</td><td><input type="text" name="mname" value="'.$a['mname'].'"></td></tr>');
